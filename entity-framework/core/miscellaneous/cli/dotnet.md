...
대상 프로젝트에는 모든 파일이 추가됩니다(또는 일부 경우 제거됨). 대상 프로젝트의 프로젝트는 현재 디렉터리에 기본값만 사용 하 여 변경할 수 있습니다 합니다 <nobr> **-프로젝트** </nobr> 옵션입니다.

시작 프로젝트는 프로젝트 코드를 실행할 때 도구가 에뮬레이트하는 프로젝트입니다. 사용 하 여 변경할 수 있습니다 하지만 기본값은 현재 디렉터리에서 프로젝트를 **-시작 프로젝트** 옵션입니다.
...
| -c | -상황에 맞는 \<DBCONTEXT >           | 사용 하 여 DbContext 합니다.       |
| -p | -프로젝트 \<프로젝트 >             | 프로젝트를 사용 하는입니다.         |
| -s | -시작 프로젝트 \<프로젝트 >     | 시작 프로젝트 사용입니다. |
|    | -framework \<프레임 워크 >         | 대상 프레임 워크입니다.       |
|    | -구성 \<구성 > | 사용할 구성입니다.   |
|    | -런타임 \<식별자 >          | 런타임 사용입니다.         |
...
|              |                                                                                              |
|:-------------|:---------------------------------------------------------------------------------------------|
| \<마이그레이션 &GT; | 대상 마이그레이션입니다. 0 인 경우, 모든 마이그레이션 되돌려집니다. 기본값 마지막 마이그레이션입니다. |
...
| \<공급자 &GT;   | 공급자에서 사용 합니다. (예: Microsoft.EntityFrameworkCore.SqlServer) |

옵션:

|                 |                                         |                                                                                                  |
|:----------------|:----------------------------------------|:-------------------------------------------------------------------------------------------------|
| <nobr>-d</nobr> | -데이터 주석                      | (가능한 한) 경우 모델을 구성 하려면 특성을 사용 합니다. 생략 하면 fluent API만 사용 됩니다. |
| -c              | -상황에 맞는 \<이름 >                       | DbContext의 이름입니다.                                                                       |
|                 | -상황에 맞는-dir \<경로 >                   | 에 DbContext 파일을 배치할 디렉터리입니다. 프로젝트 디렉터리를 기준으로 경로입니다.             |
| -f              | -force                                 | 기존 파일을 덮어씁니다.                                                                        |
| -o              | -출력 dir \<경로 >                    | 디렉터리에 파일 보관입니다. 프로젝트 디렉터리를 기준으로 경로입니다.                      |
|                 | <nobr>--schema \<SCHEMA_NAME>...</nobr> | 에 대 한 엔터티 형식을 생성 하는 테이블의 스키마입니다.                                              |
| -t              | -테이블 \<TABLE_NAME >...                | 에 대 한 엔터티 형식을 생성 하는 테이블입니다.                                                         |
|                 | -사용 하 여 데이터베이스 이름                    | 데이터베이스에서 직접 테이블 및 열 이름을 사용 합니다.                                           |
...
|:---|:-----------------|:-------------------------------------------------------------------|
| -o | -출력 \<파일 > | 결과를 쓸 파일입니다.                                   |
| -i | -멱 등 원     | 모든 마이그레이션 데이터베이스에서 사용할 수 있는 스크립트를 생성 합니다. |
