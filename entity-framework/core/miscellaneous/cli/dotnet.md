...
대상 프로젝트에는 모든 파일을 추가 하는 경우 (또는 경우에 따라 제거)입니다. 대상 프로젝트의 프로젝트는 현재 디렉터리에 기본적으로 사용 되지만 사용 하 여 변경할 수는 <nobr> **-프로젝트** </nobr> 옵션입니다.

시작 프로젝트는 프로젝트의 코드를 실행 하는 경우 도구에서 에뮬레이트됩니다. 사용 하 여 변경할 수 있지만 것 현재 디렉터리에 프로젝트에 기본적으로 **-시작 프로젝트** 옵션입니다.
...
| -- | --------- | -------------------------------------------------------- |
| -f | -force   | 확인 하지 마십시오.                                           |
|    | --예행 | 표시는 데이터베이스는 삭제할 수 없지만 삭제 하지 않습니다. |
...
|              |                                                                                              |
| ------------ | ---------------------------------------------------------------------------------------------|
| \<마이그레이션 > | 대상 마이그레이션입니다. 0 인 경우, 모든 마이그레이션 되돌립니다. 마지막으로 마이그레이션을 기본값입니다. |
...
|               |                                                                     |
| ------------- | ------------------------------------------------------------------- |
| \<연결 > | 데이터베이스에 연결 문자열입니다.                              |
| \<공급자 >   | 사용 하는 공급자입니다. 예를 들어 Microsoft.EntityFrameworkCore.SqlServer) |
...
|                 | <nobr>-스키마 \<SCHEMA_NAME >...</nobr> | 에 대 한 엔터티 형식을 생성 하는 테이블의 스키마입니다.      |
...
|                 |                                   |                                                                |
| --------------- |---------------------------------- | -------------------------------------------------------------- |
| <nobr>-o</nobr> | <nobr>-출력-dir \<경로 ></nobr> | 사용할 디렉터리 (및 하위 네임 스페이스). 경로 프로젝트 디렉터리에 상대적입니다. 기본값은 "마이그레이션"입니다. |
...
|    |         |                                                                       |
| -- | ------- | --------------------------------------------------------------------- |
| -f | -force | 마이그레이션 데이터베이스에 적용 된 경우 참조를 확인 하지 마십시오. |
...
|         |                                                               |
| ------- | ------------------------------------------------------------- |
| \<> | 마이그레이션을 시작 합니다. 기본값은 0 (초기 데이터베이스)입니다. |
| \<>   | 끝의 마이그레이션입니다. 마지막으로 마이그레이션을 기본값입니다.         |
