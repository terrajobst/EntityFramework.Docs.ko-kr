...
미리 보기 2에는 데이터베이스 스칼라 함수를 메서드 스텁에 매핑하여 LINQ 쿼리에서 사용하고 SQL로 변환할 수 있게 하는 [Paul Middleton](https://github.com/pmiddleton)이 개발한 중요한 기능이 포함되었습니다.
...
이러한 메서드는 자동으로 등록됩니다. 메서드가 등록되면 쿼리의 어느 부분에서나 사용할 수 있습니다.
...
Like()에는 메모리 구현이 다르므로, 메모리 내 데이터베이스에 대해 작업할 때나 조건자 평가가 클라이언트 측에서 필요한 경우에 유용합니다.
