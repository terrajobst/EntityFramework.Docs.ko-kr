...
---
# <a name="owned-entity-types"></a>소유 엔터티 형식
...
소유 된 엔터티 형식은 규칙에 따라 모델의 EF Core에 포함 되지 않습니다. 에서 `OwnsOne` `OwnedAttribute` 메서드를 사용 하거나 (EF Core 2.1의 새로운) 형식에 주석을 추가 하 여 형식을 소유 된 형식으로 구성할 수 있습니다. `OnModelCreating`

이 예제 `StreetAddress` 에서는 identity 속성이 없는 형식입니다. 특정 주문의 배송 주소를 지정하기 위한 Order 형식 속성으로 사용됩니다.
...
`OwnsOne` 또한에서 `OnModelCreating` 메서드를 사용 하 여 `ShippingAddress` 속성이 `Order` 엔터티 형식의 소유 된 엔터티 임을 지정 하 고 필요한 경우 추가 패싯을 구성할 수 있습니다.

[!code-csharp[OwnsOne](../../../samples/core/Modeling/OwnedEntities/OwnedEntityContext.cs?name=OwnsOne)]

속성이 형식에서 private 이면 `OwnsOne` 메서드의 문자열 버전을 사용할 수 있습니다. `Order` `ShippingAddress`
...
를 사용 하 여 `OwnsOne` 구성 하거나 참조 탐색을 통해 검색 된 형식에는 항상 소유자와 일 대 일 관계가 있으므로 외래 키 값이 고유 하므로 고유한 키 값이 필요 하지 않습니다. 이전 예제 `StreetAddress` 에서 형식은 키 속성을 정의할 필요가 없습니다.  
...
소유 된 형식의 컬렉션을 구성 하려면에서 `OwnsMany` `OnModelCreating`을 사용 합니다.

소유 된 형식에는 기본 키가 필요 합니다. .NET 형식에 적합 한 후보 속성이 없으면 하나를 만들 수 EF Core. 그러나 소유 된 형식이 컬렉션을 통해 정의 되는 경우에 대 한 `OwnsOne`것과 같이 소유 된 인스턴스의 소유자 및 기본 키로 외래 키 역할을 수행 하는 그림자 속성을 만드는 것 만으로는 충분 하지 않습니다. 각 소유자는 소유자의 키로 인해 소유 된 각 인스턴스에 대 한 고유 id를 제공 하기에 충분 하지 않습니다.

이에 대 한 가장 간단한 두 가지 솔루션은 다음과 같습니다.
- 소유자를 가리키는 외래 키와 관계 없이 새 속성에 서로게이트 기본 키를 정의 합니다. 포함 된 값은 모든 소유자에서 고유 해야 합니다 ( {1} 예: parent에 자식이 {1}있는 경우 parent {2} 는 자식을 {1}가질 수 없음). 따라서 값에 내재 된 의미가 없습니다. 외래 키가 기본 키의 일부가 아니므로 해당 값을 변경할 수 있으므로 한 부모에서 다른 부모로 자식 항목을 이동할 수 있습니다. 그러나 일반적으로 집계 의미 체계를 기반으로 합니다.
- 외래 키 및 추가 속성을 복합 키로 사용 합니다. 이제 추가 속성 값은 지정 된 부모에 대해서만 고유 해야 합니다 {1} . 따라서 부모에 자식 {1,1} 요소가 있으면 부모 {2} 는 여전히 자식을 {2,1}가질 수 있습니다. 기본 키의 외래 키 부분을 만들면 소유자와 소유 된 엔터티 간의 관계가 변경할 수 없게 되 고 집계 의미 체계가 향상 됩니다. 이 EF Core 기본적으로 수행 됩니다.

이 예제에서는 클래스를 사용 합니다 `Distributor` .

[!code-csharp[Distributor](../../../samples/core/Modeling/OwnedEntities/Distributor.cs?name=Distributor)]

기본적으로 `ShippingCenters` 탐색 속성 `("DistributorId", "Id")` 을 통해 참조 되는 소유 된 형식에 사용 되는 기본 키는 `"DistributorId"` 이며, 여기서은 `"Id"` FK 이며 고유한 `int` 값입니다.

다른 PK 호출 `HasKey`을 구성 하려면:
...
기본적으로 EF Core는 패턴 _Navigation_OwnedEntityProperty_따라 소유 된 엔터티 형식의 속성에 대 한 데이터베이스 열 이름을로 합니다. 따라서 속성 `StreetAddress` 은 이름이 ' ShippingAddress_Street ' 및 ' ShippingAddress_City ' 인 ' Orders ' 테이블에 표시 됩니다.
...
예를 들어 다음 클래스 `ShippingAddress` 에서 및 `BillingAddress` 는 모두 동일한 .net 형식 `StreetAddress`입니다.
...
이 예제 `OrderDetails` 에서는 둘 `BillingAddress` 다 `ShippingAddress` 형식인`StreetAddress` 및를 소유 합니다. 그리고 `OrderDetails`는 `DetailedOrder` 형식입니다.
...
흐름 호출에서 `OwnsOne` 메서드를 연결 하 여이 모델을 구성할 수 있습니다.

[!code-csharp[OwnsOneNested](../../../samples/core/Modeling/OwnedEntities/OwnedEntityContext.cs?name=OwnsOneNested)]

소유자에서 뒤로 가리키기 탐색 속성을 구성 하는 데 사용 되는 호출을확인합니다.`WithOwner`

`OrderDetails` `OwnedAttribute` 및`StreetAdress`를 사용 하 여 결과를 얻을 수 있습니다.

## <a name="storing-owned-types-in-separate-tables"></a>소유 형식을 별도의 테이블에 저장

또한 EF6 복합 형식과 달리 소유 된 형식은 소유자와는 별도의 테이블에 저장 될 수 있습니다. 소유 된 형식을 소유자와 동일한 테이블에 매핑하는 규칙을 재정의 하기 위해를 호출 `ToTable` 하 고 다른 테이블 이름을 제공할 수 있습니다. 다음 예에서는 및 두 `OrderDetails` 개의 주소를의 별도 `DetailedOrder`테이블에 매핑합니다.

[!code-csharp[OwnsOneTable](../../../samples/core/Modeling/OwnedEntities/OwnedEntityContext.cs?name=OwnsOneTable)]

## <a name="querying-owned-types"></a>소유 형식 쿼리

소유자를 쿼리할 때 소유된 형식은 기본적으로 포함됩니다. 소유 된 형식이 별도의 테이블에 `Include` 저장 된 경우에도 메서드를 사용할 필요가 없습니다. 앞에서 설명한 모델에 따라 다음 쿼리는 데이터베이스에서 소유 `Order` `StreetAddresses` 하 `OrderDetails` 고 두 가지를 가져옵니다.
...
- 소유 된 형식에 `DbSet<T>` 대해서는를 만들 수 없습니다.
- 에서 소유 된 `Entity<T>()` 형식을 사용 하 여를 호출할 수 없습니다.`ModelBuilder`
