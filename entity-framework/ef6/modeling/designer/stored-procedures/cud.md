...
이 단계별 연습에서는 Entity Framework Designer (EF Designer)를 사용 하 여 엔터티 형식의 create @ no__t-0insert, update 및 delete (CUD) 작업을 저장 프로시저에 매핑하는 방법을 보여 줍니다.  기본적으로 Entity Framework는 CUD 작업에 대 한 SQL 문을 자동으로 생성 하지만 저장 프로시저를 이러한 작업에 매핑할 수도 있습니다.  
...
- CUD 작업 중 하나를 저장 프로시저에 매핑하는 경우 모든 작업을 매핑합니다. 세 가지를 모두 매핑하지 않으면 매핑되지 않은 작업이 실행 될 때 실패 하 고 @no__t- **1Updateexception**이 throw 됩니다.
- 저장 프로시저의 모든 매개 변수를 엔터티 속성에 매핑해야 합니다.
- 서버에서 삽입 된 행에 대 한 기본 키 값을 생성 하는 경우이 값을 다시 엔터티의 키 속성에 매핑해야 합니다. 다음 예제에서 **Insertperson** stored 프로시저는 새로 만든 기본 키를 저장 프로시저의 결과 집합의 일부로 반환 합니다. 기본 키는 EF 디자이너의 **&lt;Add Bindings @ no__t-3**  기능을 사용 하 여 엔터티 키 (**PersonID**)에 매핑됩니다.
- 저장 프로시저 호출은 개념적 모델의 엔터티와 함께 1:1에 매핑됩니다. 예를 들어 개념적 모델에서 상속 계층 구조를 구현 하 고 **부모** (기본) 및 **자식** (파생 된) 엔터티에 대 한 CUD 저장 프로시저를 매핑하는 경우 **자식** 변경 내용을 저장 하면 **자식**만 호출 됩니다. s 저장 프로시저는 **부모**저장 프로시저 호출을 트리거하지 않습니다.
...
-  **새 연결**을 클릭 합니다. 연결 속성 대화 상자에서 서버 이름 (예: **(localdb)\\mssqllocaldb**)을 입력 하 고, 인증 방법을 선택 하 고, 데이터베이스 이름으로 **School** 을 입력 한 다음, **확인**을 클릭 합니다.
    데이터 연결 선택 대화 상자가 데이터베이스 연결 설정으로 업데이트 됩니다.
- 데이터베이스 개체 선택 대화 상자에서 **테이블** node 아래에 있는 **Person** 테이블을 선택 합니다.
- 또한 **저장 프로시저 및 함수** 노드에서 다음 저장 프로시저를 선택 합니다. **Deleteperson**, **Insertperson**및 **updateperson**.
...
-  **@No__t-1Select 함수 @ no__t-2를 선택**합니다.
    필드가 스토리지 모델의 저장 프로시저를 포함하는 드롭다운 목록이 됩니다. 이 목록의 저장 프로시저를 개념적 모델의 엔터티 형식에 매핑할 수 있습니다.
    드롭다운 목록에서 **Insertperson** 을 선택 합니다.
- 저장 프로시저 매개 변수와 엔터티 속성 간의 기본 매핑이 나타납니다. 화살표는 매핑 방향을 나타냅니다. 속성 값이 저장 프로시저 매개 변수에 제공됩니다.
-  **@No__t-1Add 바인딩 추가 @ no__t-2**를 클릭 합니다.
-  **NewPersonID**를 입력 하 고 **insertperson** stored 프로시저에서 반환 된 매개 변수의 이름을 입력 합니다. 선행 또는 후행 공백을 입력 하지 않아야 합니다.
-  **Enter**키를 누릅니다.
- 기본적으로 **NewPersonID** 은 엔터티 키 **PersonID**에 매핑됩니다. 화살표는 매핑 방향을 나타냅니다. 결과 열의 값이 속성에 제공됩니다.

    ![매핑 정보](~/ef6/media/mappingdetails.png)

- @No__t-1을 클릭 하 고 **업데이트 함수 @ no__t-2** 을 선택 하 고 결과 드롭다운 목록에서 **updateperson** 를 선택 합니다.
- 저장 프로시저 매개 변수와 엔터티 속성 간의 기본 매핑이 나타납니다.
-  **@No__t-1Select 함수 @ no__t-2** 을 선택 하 고 결과 드롭다운 목록에서 **deleteperson** 를 선택 합니다.
- 저장 프로시저 매개 변수와 엔터티 속성 간의 기본 매핑이 나타납니다.
...
- **OUTPUT** 매개 변수를 사용 하 여 저장 프로시저에서 영향을 받는 행 수를 반환 하 고 매개 변수 이름 옆에 있는   확인란의 **영향을 받은 행 매개 변수**를 확인 합니다. 작업이 호출 될 때 반환 되는 값이 0 이면  [**OptimisticConcurrencyException**](https://msdn.microsoft.com/library/system.data.optimisticconcurrencyexception.aspx) 가 throw 됩니다.
